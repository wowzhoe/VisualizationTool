...
public class RMF_RadialMenu : MonoBehaviour {
    ...
    public enum Enviorment_Type {VR, Desktop}

    public Enviorment_Type enviorment_Type = Enviorment_Type.Desktop;
    ...
    void Start() {
        ...
        if (useGamepad) {
            ...
            if (useSelectionFollower && selectionFollowerContainer != null)
                selectionFollowerContainer.localRotation = Quaternion.Euler(selectionFollowerContainer.localRotation.eulerAngles.x, selectionFollowerContainer.localRotation.eulerAngles.y, -globalOffset); //Point the selection follower at the first element.
        }
    ...
    void Update() {
        ...
        //==============================================================================================
        bool joystickMoved = OVRInput.Get(OVRInput.Axis2D.SecondaryThumbstick)[1] != 0.0 || OVRInput.Get(OVRInput.Axis2D.SecondaryThumbstick)[0] != 0.0;
        //==============================================================================================
        ...
        if (!useGamepad) {
            ...
            if (enviorment_Type == Enviorment_Type.Desktop) {
                rawAngle = Mathf.Atan2(Input.GetAxis("Vertical"),Input.GetAxis("Horizontal")) * Mathf.Rad2Deg;
            }else if(enviorment_Type == Enviorment_Type.VR) {
                rawAngle = Mathf.Atan2(OVRInput.Get(OVRInput.Axis2D.SecondaryThumbstick)[1],OVRInput.Get(OVRInput.Axis2D.SecondaryThumbstick)[0]) * Mathf.Rad2Deg;
            }
        }
        ...
        if (angleOffset != 0 && useLazySelection) {
            ...
            if (elements[index] != null) {
                ...
                if (enviorment_Type == Enviorment_Type.Desktop) {
                    submitSelection = (Input.GetMouseButtonDown(0) || Input.GetButtonDown("Submit"));
                } else if (enviorment_Type == Enviorment_Type.VR) {
                    submitSelection = (Math.Abs(OVRInput.Get(OVRInput.Axis2D.SecondaryThumbstick)[1]) < 0.25 && Math.Abs(OVRInput.Get(OVRInput.Axis2D.SecondaryThumbstick)[0]) < 0.25);
                }
        ...
        if (useSelectionFollower && selectionFollowerContainer != null) {
            if (!useGamepad || joystickMoved)
                selectionFollowerContainer.localRotation = Quaternion.Euler(selectionFollowerContainer.localRotation.eulerAngles.x,selectionFollowerContainer.localRotation.eulerAngles.y, rawAngle + 270);
