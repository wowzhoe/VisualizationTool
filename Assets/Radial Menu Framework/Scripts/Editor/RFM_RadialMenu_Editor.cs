    ...
    public void arrangeElementsInEditor(RMF_RadialMenu rm, bool reset) {
        ...
        if (reset) {
            ...
            for (int i = 0; i < rm.elements.Count; i++) {
                ...
                elemRt.localRotation = Quaternion.Euler(elemRt.localRotation.eulerAngles.x,elemRt.localRotation.eulerAngles.y, 0);
        ...
        for (int i = 0; i < rm.elements.Count; i++) {
            ...
            RectTransform elemRt = rm.elements[i].GetComponent<RectTransform>();
            elemRt.localRotation = Quaternion.Euler(elemRt.localRotation.eulerAngles.x,elemRt.localRotation.eulerAngles.y, -((360f / (float)rm.elements.Count) * i) - rm.globalOffset);
